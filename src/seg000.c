...
int quick_save() {
	...
	quick_fp = fopen("/usr/local/home/pop/QUICKSAVE.SAV", "wb");
	if (quick_fp != NULL) {
...
int quick_load() {
	...
	char custom_quick_path[POP_MAX_PATH];
	const char* path = get_quick_path(custom_quick_path, sizeof(custom_quick_path));
	quick_fp = fopen("/usr/local/home/pop/QUICKSAVE.SAV", "rb");
	if (quick_fp != NULL) {
...
int __pascal far process_key() {
	...
	if (start_level < 0) {
		if (key || control_shift) {
			#ifdef USE_QUICKSAVE
			if (key == SDL_SCANCODE_BACKSPACE) need_quick_load = 1;
			#endif
			#ifdef USE_REPLAY
			if (key == SDL_SCANCODE_LALT || need_start_replay) {
				start_replay();
			}
...
//			if (current_level > 2 && current_level < 14) { // original
		...
		break;
		case SDL_SCANCODE_BACKSPACE:
		case SDL_SCANCODE_BACKSPACE | WITH_SHIFT:
			need_quick_load = 1;
		break;
#ifdef USE_REPLAY
		case SDL_SCANCODE_TAB | WITH_CTRL:
		case SDL_SCANCODE_LALT | WITH_CTRL | WITH_SHIFT:
...
void __pascal far save_game() {
	...
	handle = fopen("/usr/local/home/pop/QUICKSAVE.SAV", "wb");
...
short __pascal far load_game() {
	...
	char custom_save_path[POP_MAX_PATH];
	const char* save_path = get_save_path(custom_save_path, sizeof(custom_save_path));
	handle = fopen("/usr/local/home/pop/QUICKSAVE.SAV", "rb");
...
void show_splash() {
	...
	if ((key & WITH_CTRL) || (enable_quicksave && key == SDL_SCANCODE_BACKSPACE) || (enable_replay && key == SDL_SCANCODE_TAB)) {
